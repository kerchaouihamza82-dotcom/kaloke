      ...
      <main className="container mx-auto px-6 py-24">
        <div className="mx-auto max-w-5xl">
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            {/* Plan Mensual */}
            <Card className={`relative flex flex-col border-2 transition-all cursor-pointer ${selectedPlan === 'mensual' ? 'border-blue-500' : 'border-blue-500/40'}`}
              ...
              <CardContent className="flex flex-col gap-6 p-8 pt-10">
                ...
                {selectedPlan !== 'mensual' && (
                  <Button className="w-full bg-blue-600 py-5 text-base hover:bg-blue-700" onClick={e => { e.stopPropagation(); onSelectPlan('mensual') }}>
                    Elegir este plan
                  </Button>
                )}
              </CardContent>
            ...
            {/* Plan Anual */}
            <Card className={`flex flex-col border-2 transition-all cursor-pointer ${selectedPlan === 'anual' ? 'border-foreground' : 'border-border'}`}
              ...
              <CardContent className="flex flex-col gap-6 p-8">
                ...
                {selectedPlan !== 'anual' && (
                  <Button variant="outline" className="w-full border-foreground py-5 text-base hover:bg-foreground hover:text-background" onClick={e => { e.stopPropagation(); onSelectPlan('anual') }}>
                    Elegir este plan
                  </Button>
                )}
              </CardContent>
                  ...
                  <form onSubmit={onSubmit} className="flex flex-col gap-4">
                    <div className="space-y-1 text-center">
                      <h3 className="text-lg font-light">
                        Plan {selectedPlan === 'mensual' ? 'Mensual — $9.99/mes' : 'Anual — $2,500/año'} seleccionado
                      </h3>
                      <p className="text-sm text-muted-foreground">Introduce tu email para continuar al pago</p>
                    </div>
                    <Input
                      type="email"
                    ...
                    <Button type="submit" className="w-full py-5 text-base" disabled={!!loadingPlan}>
                      {loadingPlan ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Redirigiendo al pago...</> : 'Continuar al pago seguro'}
                    </Button>
